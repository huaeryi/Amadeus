cognition 机制：
- `amds_state/cognition.json` 是 metadata、facts、state、capabilities 的机器可读 source of truth；`amds_state/COGNITION.md` 只能由 `{{root_name}}/bin/state_docs.py render {{challenge_path}}` 生成
...
- 如果题目目录没有 `amds_state/cognition.json`，运行 `{{root_name}}/bin/state_docs.py init {{challenge_path}}`
