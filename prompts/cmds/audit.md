...
6. 已确认事实写入 `cognition.json.facts`，审计假设、候选漏洞、失败路线和下一步写入 `cognition.json.state`
...
8. 长命令输出、调试日志、PoC 输出、HTTP transcript、测试日志和截图等证据文件统一保存到 `{{challenge_path}}/evidence/`，在 `cognition.json` 中只写摘要和相对路径
