...
```bash
{{root_name}}/bin/amds --mode solve --workflow <pwn|web|crypto|reverse|misc|x402> --runner {{runner}} --policy {{policy}} [--append <用户描述或补充信息>] {{challenge_path}}
```
...
输出要求：
- 第一行只输出最终命令，不要加解释；必须包含 `--mode solve`、`--workflow <pwn|web|crypto|reverse|misc|x402>`、`--runner {{runner}}`、`--policy {{policy}}` 和 `{{challenge_path}}`
- 如果用户通过位置参数或 `--append` 提供了题目描述、远程地址、题型提示或命令片段，把这些信息作为 `--append` 参数保留在最终命令中
