capabilities 机制：
- `capabilities.json` 是机器可读 source of truth；`CAPABILITIES.md` 只能由 `{{root_name}}/bin/capabilities.py render {{challenge_path}}` 生成
- 如果题目目录没有 `capabilities.json`，运行 `{{root_name}}/bin/capabilities.py init {{challenge_path}}`
...
- runtime/gdb/pwndbg/IDA/checksec/脚本输出都可以作为 evidence，但必须能复现或定位
...
- 不要只在 `CAPABILITIES.md` 写结论；所有结论必须先进 `capabilities.json`
