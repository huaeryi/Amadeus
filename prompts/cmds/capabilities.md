capabilities 机制：
- `cognition.json.capabilities` 是机器可读 source of truth；`COGNITION.md` 只能由 `{{root_name}}/bin/capabilities.py render {{challenge_path}}` 生成
- 如果题目目录没有 `cognition.json`，运行 `{{root_name}}/bin/capabilities.py init {{challenge_path}}`
...
- runtime/gdb/pwndbg/IDA/checksec/脚本输出都可以作为 evidence，但必须能复现或定位；需要保存的输出统一写入 `evidence/`，artifact 使用相对路径如 `evidence/checksec.txt`
...
- 不要只在 `COGNITION.md` 写结论；所有结论必须先进 `cognition.json.capabilities`
