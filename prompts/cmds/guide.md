...
4. 如果我暂时不回答，而你继续推进，必须在 `amds_state/cognition.json.state` 的 `your_turn` 记录我应该补想的判断题和当前正确答案暂不展开的部分，再渲染 `amds_state/COGNITION.md`。
...
产物要求：
- 在 `amds_state/cognition.json.state` 中维护 `your_turn`，保留 3 到 5 个当前阶段我应该能回答的问题，并渲染到 `amds_state/COGNITION.md` 的 `Your Turn` 小节。
