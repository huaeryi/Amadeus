...
本次 CTF 方向由 `--workflow {{workflow}}` 决定；具体题型流程见后续 `{{root_name}}/prompts/workflow/{{workflow}}.md` 内容，不要根据题目名或旧经验擅自切换方向。若实际材料明显不属于该方向，先在 `amds_state/cognition.json.state` 记录证据和 pivot 原因，再渲染 `amds_state/COGNITION.md` 后切换思路。
