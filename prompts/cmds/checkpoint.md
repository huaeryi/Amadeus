...
- 创建 checkpoint 前先确认 `facts.json`、`state.json`、`capabilities.json`、`exp.py`/`solve.py`/`wp.md` 等关键文件已写入当前结论，并运行 `{{root_name}}/bin/state_docs.py render {{challenge_path}}` 和 `{{root_name}}/bin/capabilities.py render {{challenge_path}}`
