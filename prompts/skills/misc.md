...
这题主要按 misc 路线推进，不要偏离主线；除非题目材料明确证明分类错误，否则 CTF 方向以 `--workflow misc` 为准。如果确认题目实际属于 crypto/web/reverse/pwn，先在 `state.json` 记录 pivot 原因并渲染 `STATE.md`，再切换对应技能思路。
...
5. 如果某条路线失败，不要在脏状态上持续修补；在 `state.json` 的 rejected_branches 记录失败原因，并按公共 checkpoint 策略回退或开分支
