...
behavior:
  - reads optional amds_state/run.env, falling back to legacy .pwnrun files
...
config_path="$challenge_dir/amds_state/run.env"
if [ ! -f "$config_path" ] && [ -f "$challenge_dir/amds_state/.pwnrun" ]; then
...
if [ "$mode" = "remote" ]; then
    ...
    if [ "$REMOTE_APPEND_HOSTPORT" = "1" ]; then
        [ -n "$host" ] || die "remote host missing; pass it on the command line or set HOST in amds_state/run.env"
        [ -n "$port" ] || die "remote port missing; pass it on the command line or set PORT in amds_state/run.env"
    fi
fi
