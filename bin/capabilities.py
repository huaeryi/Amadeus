...
TEMPLATE = ROOT / "templates" / "capabilities.json"
...
def capability_paths(challenge_dir: Path) -> tuple[Path, Path]:
    return challenge_dir / "capabilities.json", challenge_dir / "CAPABILITIES.md"
...
def load_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise CapabilityError("capabilities.json must contain a JSON object")
    return data
...
def empty_document(challenge_dir: Path) -> dict[str, Any]:
    data = load_json(TEMPLATE)
    data["challenge"] = challenge_dir.name
    data["active_env"] = data.get("active_env") or "local"
    data["capabilities"] = []
    return data
...
def ensure_document(challenge_dir: Path) -> dict[str, Any]:
    ...
    data = empty_document(challenge_dir)
...
def render_markdown(data: dict[str, Any]) -> str:
    lines: list[str] = [
        ...
        f"- source: capabilities.json",
        "",
        "> Generated from `capabilities.json`. Do not edit this file directly.",
        "",
    ]
...
def command_init(challenge_dir: Path) -> int:
    ...
    data = ensure_document(challenge_dir)
